...
また、本研究内容について学会において発表をし、以下のような質疑応答がなされた。

「死活監視、トラフィック監視を統合したいというのは具体的にどういうことか、問題点がよく分からない。アラートが大量に鳴りすぎて良く分からなくなるということか。死活監視は機器が壊れている、トラフィック監視は持ってる帯域が足りてない。それぞれ原因が違うから統合したいというのはどういうことなのか。」という指摘があった。これに対し、「統合したい理由としては、インフラ監視は運用者が機器の状態を直接見るが、品質監視はエンドユーザー側の視点として監視をする。それらは現在別々で監視をするから、それを統合したい。」と説明した。
...
更に、「LLMを使わなければならない理由は何だろうか。システムが出すエラーは有限だから、わざわざLLM使うよりそれに適したネットワークなどを構築して学習させたほうが良いのでは。RAGを使って高速化した結果を使ってネットワークを構築した結果を学習させたほうが良いのでは」という指摘もなされた。この点については、「それについては検討します。」と回答し、今後、ルールベース手法や従来の機械学習手法、RAG等との比較を通じて、LLMを用いる必然性と有効性を検証することを今後の課題とする。

このような学会発表時の質疑応答を通じて、本研究における監視情報統合の目的は運用者視点のインフラ監視とエンドユーザー視点の品質監視を横断的に関連付けることにあると明確になった。あわせて、障害とインシデントの関係を整理し、運用判断を支援する基盤を構築する必要性が示された。
